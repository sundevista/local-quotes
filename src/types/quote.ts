...
import {
  getRandomArrayItem,
  getRandomAuthor,
  getRandomQuoteOfAuthor,
  getWeightedRandomAuthor,
} from "../utils/random";
...
export function getValidAuthorsFromAdvancedSearch(
  quoteVault: Quote[],
  search: string
): string[] {
...
): BlockMetadataContent {
  const result: BlockMetadataContent = { author: null, text: null };
...
export function appendToLastQuote(
  quoteVault: Quote[],
  filename: string,
  author: string,
  text: string
  ...
  const filesQuotesIdx: number = getFilesQuotesIdx(
    quoteVault,
    filename,
    author
  );
  const quoteIdx: number =
    quoteVault[authorIdx].files[filesQuotesIdx].quotes.length - 1;

  quoteVault[authorIdx].files[filesQuotesIdx].quotes[quoteIdx] =
    quoteVault[authorIdx].files[filesQuotesIdx].quotes[quoteIdx] + "\n" + text;
}

export async function updateQuotesVault(
  plugin: LocalQuotes,
  files: TFile[]
): Promise<void> {
  const tmpQuoteVault: Quote[] = [];
