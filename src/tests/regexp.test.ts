...
test("checking search_regexp", () => {
  const cases = [
    ...
    [
      "First Person, with comma. and dot || Second Person || And, finally, third",
      true,
    ],
    ["Person", true],
...
test("checking code_block_id_regexp", () => {
  ...
  for (const _case of cases) {
    expect(testRegExp(_case[0].toString(), code_block_id_regexp)).toBe(
      _case[1]
    );
  }
});
...
test("checking code_block_search_regexp", () => {
  const cases = [
    ...
    [
      "search First Person, with comma. and dot || Second Person || And, finally, third",
      true,
    ],
    ["search Person", true],
  ...
  for (const _case of cases) {
    expect(testRegExp(_case[0].toString(), code_block_search_regexp)).toBe(
      _case[1]
    );
  }
});
...
test("checking code_block_refreshInterval_regexp", () => {
  ...
  for (const _case of cases) {
    expect(
      testRegExp(_case[0].toString(), code_block_refreshInterval_regexp)
    ).toBe(_case[1]);
  }
});
...
test("checking code_block_customClass_regexp", () => {
  ...
  for (const _case of cases) {
    expect(testRegExp(_case[0].toString(), code_block_customClass_regexp)).toBe(
      _case[1]
    );
  }
});
